\begin{figure}[t]
  ...
  \caption{Scalar flux ($\log_{10}$) for the lattice test case  at
    $t=3.2 $ seconds using the octahedron (lerp) quadrature.  The
    spatial domain is $[0 \text{ cm}, 7\text{ cm}]^2$ with $n_x=n_y =
    280$. Rows vary in $n_q$ and columns vary in $\delta$,
    respectively. Isolines are drawn to highlight the values $-5$
  (white), $-4$ (gray), and $-3$ (black).}
  \label{fig:rsn:cb}
\end{figure}
